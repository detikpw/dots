...
# wallpaper randomizer that I get from
...
if [ $COUNT -ge $MINCOUNT ]; then
    ...
    convert "$NEW" -thumbnail 50x50^ -gravity Center -crop 50x50-0-0 "$CONFIG/i3/thumb.png"
    ...
    nitrogen --set-scaled $NEW && { notify-send -t 5000 -i "$CONFIG/i3/thumb.png" "Nitrogenium" "Changed wallaper to \"$NAME\""; sed -i "s!^file=.*!file=$NEW!" $CONFIG/nitrogen/bg-saved.cfg; }

    # remove thumbnail
    rm -f "$CONFIG/i3/thumb.png"
else
