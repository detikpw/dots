...
documents.onDidOpen((event) => {
    ...
    if (!document2Library[event.document.uri]) {
        ...
        if (uri.scheme === 'file') {
            ...
            if (nodePath) {
                promise = vscode_languageserver_1.Files.resolve(style, nodePath, nodePath, trace).then(undefined, () => {
                    return vscode_languageserver_1.Files.resolve(style, globalNodePath, directory, trace);
                });
            }
            else {
                promise = vscode_languageserver_1.Files.resolve(style, globalNodePath, directory, trace);
            }
        }
        else {
            promise = vscode_languageserver_1.Files.resolve(style, globalNodePath, workspaceRoot, trace);
        }
...
function validate(document, library, publishDiagnostics = true) {
    let newOptions = Object.assign(Object.create(null), { filename: document.uri }, options);
    ...
    try {
        ...
        deglob([file], deglobOpts, function (err, files) {
            ...
            if (files.length === 1) {
                ...
                library.lintText(content, newOptions, function (error, report) {
                    if (error) {
